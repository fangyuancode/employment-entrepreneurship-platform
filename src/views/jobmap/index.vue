  ...
  <div class="job-map-page" :class="{ 'is-enter-ready': enterReady, 'is-clean-mode': cleanMode }">
    ...
    <header class="map-header">
      ...
      <div class="header-right">
        ...
        <el-button class="screen-btn" size="small" :loading="loading" @click="loadGraphData"
          >刷新</el-button
        >
        <el-button class="screen-btn" size="small" @click="toggleFullScreen">全屏</el-button>
        <el-button class="screen-btn" size="small" @click="toggleCleanMode">{{
          cleanMode ? '恢复面板' : '清屏展示'
        }}</el-button>
    ...
    <section class="filter-bar">
      ...
      <el-form :inline="true" :model="queryForm" class="filter-form">
        ...
        <el-form-item class="filter-action-item">
          ...
          <el-button class="screen-btn" @click="toggleCleanMode">{{
            cleanMode ? '恢复面板' : '清屏'
          }}</el-button>
          <el-button class="screen-btn" @click="toggleDemoMode">{{
            demoMode ? '停止演示' : '自动演示'
          }}</el-button>
          <el-button
    ...
    <main
      v-loading="loading"
      class="map-body"
      element-loading-text="3D技能图谱构建中..."
      element-loading-background="rgba(3, 12, 30, 0.72)"
    >
      <aside class="side-panel side-panel--left panel-enter panel-enter--left">
            ...
            <div v-for="(item, index) in graphData.topSkills" :key="item.name" class="rank-item">
              ...
              <div class="rank-main">
                ...
                <div class="rank-track">
                  <div
                    class="rank-bar"
                    :style="{ width: getPercent(item.value, maxSkillValue) + '%' }"
                  ></div>
                </div>
              </div>
            </div>
            <el-empty
              v-if="!graphData.topSkills.length"
              description="暂无技能数据"
              :image-size="80"
            />
          </div>
        ...
        <section class="panel-card panel-card--compact">
          ...
          <div class="category-list">
            <div
              v-for="item in graphData.categoryDistribution"
              :key="item.name"
              class="category-item"
            >
              <span>{{ item.name }}</span>
            ...
            <el-empty
              v-if="!graphData.categoryDistribution.length"
              description="暂无分类数据"
              :image-size="70"
            />
          </div>
      ...
      <section class="galaxy-panel panel-enter panel-enter--center">
        <div class="galaxy-toolbar">
          ...
          <div class="galaxy-tip"
            >拖拽旋转 · 滚轮缩放 · 双击复位 · 点击节点自动聚焦 · 自动演示可串联讲解</div
          >
        </div>
        ...
        <div class="floating-label-layer">
          <div
            v-for="item in floatingLabels"
            :key="item.id"
            class="floating-label"
            :class="{ 'is-active': item.active }"
            :style="{ left: item.x + 'px', top: item.y + 'px', borderColor: item.color }"
          >
        ...
        <CleanNodeAnalysisCard
          v-if="cleanMode && showCleanAnalysisCard"
          :node="selectedNode"
          :node-color="selectedNodeColor"
          :relations="selectedRelations"
          :enhanced-relations="enhancedRelations"
          :loading="cleanAiLoading"
          :summary="cleanAiSummary"
          :bullets="cleanAiBullets"
          @close="showCleanAnalysisCard = false"
          @analyze="handleCleanAiAnalyze"
          @recommend="handleRecommendFromCleanCard"
        />
      ...
      <aside class="side-panel side-panel--right panel-enter panel-enter--right">
        ...
        <section class="panel-card ai-card">
          ...
          <p class="ai-desc"
            >默认展示本地规则说明，点击后仅增强当前节点的少量关系，避免拖慢页面加载。</p
          >
          <el-button
            class="screen-btn screen-btn--primary ai-btn"
            :disabled="!selectedNode || !selectedRelations.length"
            :loading="enhanceLoading"
            @click="handleEnhanceRelations"
          >
            增强当前关系说明
          </el-button>
        ...
        <section class="panel-card recommend-card">
          ...
          <el-form label-position="top" :model="recommendForm" class="recommend-form">
            ...
            <el-form-item label="已有技能">
              <el-input
                ...
                clearable
                placeholder="Vue, TypeScript, ECharts"
              />
            </el-form-item>
          ...
          <div v-if="recommendResult.summary" class="recommend-result">
            ...
            <div class="tag-group">
              <span v-for="item in recommendResult.coreSkills" :key="'core-' + item">{{
                item
              }}</span>
            </div>
            <div class="missing-title">建议补齐</div>
            <div class="tag-group tag-group--warning">
              <span v-for="item in recommendResult.missingSkills" :key="'missing-' + item">{{
                item
              }}</span>
            </div>
          </div>
...
<script setup lang="ts">
  ...
  const maxSkillValue = computed(() =>
    Math.max(...graphData.topSkills.map((item) => item.value), 1)
  )
  ...
  const releasePointerCaptureSafely = (event: PointerEvent) => {
    ...
    try {
      ...
      // 部分移动端浏览器在滚动取消后释放 pointer capture 会报错，这里忽略即可。
    }
  }

  const shouldLetPageScroll = (event: PointerEvent, totalDx: number, totalDy: number) => {
    if (!isTouchPointer(event)) return false
    const absX = Math.abs(totalDx)
    const absY = Math.abs(totalDy)
    return absY > 8 && absY > absX * 1.15
  }

  const shouldStartSceneDrag = (event: PointerEvent, totalDx: number, totalDy: number) => {
    const absX = Math.abs(totalDx)
    const absY = Math.abs(totalDy)
    if (isTouchPointer(event)) {
      // 移动端把纵向滑动留给页面滚动，横向滑动才旋转星图。
      return absX > 8 && absX > absY * 1.08
    }
    return Math.hypot(totalDx, totalDy) > 4
  }
  ...
  const initThree = () => {
    ...
    // 允许移动端在 3D 画布上纵向滑动页面，横向滑动仍用于旋转星图。
  ...
  const handlePointerDown = (event: PointerEvent) => {
    ...
    // 桌面端立即捕获指针，移动端等确认是“横向旋转”后再捕获。
    // 这样用户在图谱区域向上/向下滑动时，页面可以正常滚动到下方详情内容。
    if (!isTouchPointer(event)) {
  ...
  const handlePointerMove = (event: PointerEvent) => {
    if (dragState.isDown && dragState.pointerId === event.pointerId) {
      ...
      if (!dragState.isDragging && shouldLetPageScroll(event, totalDx, totalDy)) {
        tooltip.visible = false
        setHoveredNode(null)
      ...
      if (!dragState.isDragging && shouldStartSceneDrag(event, totalDx, totalDy)) {
    ...
    // 移动端不需要 hover 提示，避免轻触/滚动时频繁触发射线检测和悬浮层。
  ...
  const buildCleanAiSummary = (node: SceneNode, enhanced: GraphLinkItem[]) => {
    ...
    const relationText = relatedNames.length
      ? ` 重点关联对象包括：${relatedNames.join('、')}。`
      : ''
  ...
  const updateSceneMotion = (elapsed: number, delta: number) => {
    ...
    relationParticles.forEach((particle) => {
      ...
      particle.userData.progress =
        (particle.userData.progress + particle.userData.speed * delta) % 1
      particle.position.copy(curve.getPointAt(particle.userData.progress))
    ...
    sceneLinks.forEach((link, index) => {
      ...
      material.opacity = Math.max(
        0.04,
        base * (0.86 + Math.sin(elapsed * 1.6 + index * 0.21) * 0.14)
      )
    })
  }
    ...
    if (galaxyGroup) {
      ...
      if (dragState.isDragging) {
        ...
        galaxyGroup.rotation.x +=
          (Math.sin(elapsed * 0.28) * 0.065 - galaxyGroup.rotation.x) * 0.018
        galaxyGroup.rotation.z +=
          (Math.sin(elapsed * 0.18) * 0.025 - galaxyGroup.rotation.z) * 0.014
      }
    }
...
<style scoped lang="scss">
  .job-map-page {
    ...
    color: #dff4ff;
    background:
      radial-gradient(circle at 12% 10%, rgb(31 109 180 / 22%), transparent 34%),
      radial-gradient(circle at 86% 20%, rgb(91 65 170 / 18%), transparent 32%), #061936;
  }
  ...
  .map-grid-bg {
    ...
    pointer-events: none;
    background-image:
      linear-gradient(rgb(125 190 255 / 5.5%) 1px, transparent 1px),
      linear-gradient(90deg, rgb(125 190 255 / 5.5%) 1px, transparent 1px);
  ...
  .map-header {
    ...
    padding: 14px 18px;
    margin: 16px 16px 0;
    background: rgb(8 31 58 / 78%);
    backdrop-filter: blur(14px);
    border: 1px solid rgb(104 189 255 / 18%);
    border-radius: 18px;
  ...
  .system-title {
    font-size: 25px;
    font-weight: 800;
    text-shadow: 0 0 16px rgb(56 214 255 / 45%);
    letter-spacing: 4px;
  }
  ...
  .system-line {
    ...
    height: 2px;
    margin: 10px auto 0;
    background: linear-gradient(90deg, transparent, rgb(56 214 255 / 90%), transparent);
  }
  ...
  .screen-btn {
    color: #dff4ff;
    background: rgb(13 42 75 / 58%);
    border-color: rgb(104 189 255 / 32%);
  }

  .screen-btn:hover {
    color: #fff;
    background: rgb(24 81 128 / 72%);
    border-color: rgb(56 214 255 / 78%);
  }

  .screen-btn--primary {
    background: rgb(21 102 157 / 86%);
    border-color: rgb(56 214 255 / 82%);
  }
  ...
  .filter-bar {
    ...
    padding: 12px 16px;
    margin: 12px 16px 0;
    background: rgb(8 31 58 / 76%);
    border: 1px solid rgb(104 189 255 / 16%);
    border-radius: 16px;
  }
  ...
  .filter-form :deep(.el-form-item__label) {
    color: rgb(223 244 255 / 78%);
  }

  .filter-form :deep(.el-input__wrapper),
  .filter-form :deep(.el-select__wrapper) {
    width: 170px;
    background: rgb(8 27 51 / 80%);
    border: 1px solid rgb(104 189 255 / 20%);
    box-shadow: none;
  }
  ...
  .kpi-item {
    padding: 12px;
    background: rgb(14 48 86 / 68%);
    border: 1px solid rgb(104 189 255 / 14%);
    border-radius: 14px;
  }
  ...
  .rank-item i {
    ...
    color: #38d6ff;
    text-align: center;
    background: rgb(56 214 255 / 8%);
    border: 1px solid rgb(56 214 255 / 22%);
    border-radius: 9px;
  }
  ...
  .category-progress i {
    display: block;
    height: 100%;
    background: linear-gradient(90deg, rgb(56 214 255 / 45%), rgb(56 214 255 / 95%));
  ...
  .graph-tooltip {
    ...
    padding: 10px 12px;
    pointer-events: none;
    background: rgb(3 12 30 / 86%);
    border: 1px solid rgb(56 214 255 / 35%);
    border-radius: 12px;
  ...
  .floating-label {
    ...
    background: rgb(4 18 36 / 78%);
    backdrop-filter: blur(8px);
    border: 1px solid rgb(95 231 255 / 45%);
    border-radius: 999px;
  ...
  .floating-label.is-active {
    ...
    padding: 7px 11px;
    font-size: 13px;
    background: rgb(8 32 62 / 92%);
    box-shadow: 0 10px 30px rgb(95 231 255 / 20%);
  }
  ...
  .node-profile {
    ...
    padding: 12px;
    margin-bottom: 12px;
    background: rgb(9 34 64 / 58%);
    border: 1px solid rgb(104 189 255 / 15%);
    border-radius: 14px;
  }
  ...
  .enhance-item {
    padding: 10px;
    margin-bottom: 9px;
    background: rgb(9 34 64 / 44%);
    border: 1px solid rgb(104 189 255 / 12%);
    border-radius: 12px;
  }
  ...
  .recommend-result {
    padding-top: 10px;
    margin-top: 10px;
    border-top: 1px solid rgb(104 189 255 / 12%);
  }
  ...
  .tag-group span {
    ...
    color: #dff4ff;
    background: rgb(56 214 255 / 8%);
    border: 1px solid rgb(56 214 255 / 20%);
    border-radius: 999px;
  }
  ...
  .missing-title {
    margin: 10px 0 7px;
    font-size: 12px;
    color: rgb(223 244 255 / 56%);
  }

  .tag-group--warning span {
    background: rgb(255 183 94 / 8%);
    border-color: rgb(255 183 94 / 26%);
  }
  ...
  .demo-guide {
    ...
    padding: 12px 16px;
    pointer-events: none;
    background: rgb(5 19 43 / 62%);
    backdrop-filter: blur(10px);
    border: 1px solid rgb(95 231 255 / 22%);
    border-radius: 14px;
  ...
  .demo-guide em {
    ...
    color: #7ee8ff;
    text-align: center;
    background: rgb(95 231 255 / 10%);
    border: 1px solid rgb(95 231 255 / 20%);
    border-radius: 999px;
  }

  .demo-guide.is-playing {
    background: rgb(6 28 50 / 76%);
    border-color: rgb(130 242 186 / 42%);
  }

  .demo-guide.is-playing em {
    color: #82f2ba;
    background: rgb(130 242 186 / 10%);
    border-color: rgb(130 242 186 / 30%);
  }
